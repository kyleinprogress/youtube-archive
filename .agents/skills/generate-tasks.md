...
- **Filename:** `tasks-##-[feature-name].md` (e.g., `tasks-04-user-profile-editing.md`). The `##` is a zero-padded two-digit number matching the PRD this task list derives from (so PRD #4's tasks are `tasks-04-...`). Keeps the `.agents/prd/` and `.agents/tasks/` directories visually aligned.
...
8.  **Save Task List:** Save the generated document in the `/.agents/tasks/` directory with the filename `tasks-##-[feature-name].md`, where `##` is the zero-padded number of the PRD this task list derives from and `[feature-name]` describes the main feature or task being implemented (e.g., tasks for PRD #4 about user profile editing become `tasks-04-user-profile-editing.md`).
